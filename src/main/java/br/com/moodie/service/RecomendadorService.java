...
public class RecomendadorService {
    ...
    public List<Recomendacao> recomendar(Usuario usuario, int topN) {
        ...
        List<Recomendacao> recomendacoes = gerarRecomendacaoOrdenadas(filmesFiltrados, usuario, topN, filmesDisponiveis);
    ...
    public Optional<Recomendacao> recomendarAleatorio(Usuario usuario) {
        ...
        Recomendacao recomendacao = sortearRecomendacao(filmesFiltrados, usuario, filmesDisponiveis);
    ...
    private List<Recomendacao> gerarRecomendacaoOrdenadas(List<Filme> filmes, Usuario usuario, int topN, List<Filme> catalogoBase) {
        return filmes.stream()
                .map(filme -> criarRecomendacao(filme, usuario, catalogoBase))
                .sorted(this::compararRecomendacoes)
                .limit(topN)
                .collect(Collectors.toList());
    }
    
    private Recomendacao criarRecomendacao(Filme filme, Usuario usuario, List<Filme> catalogoBase) {
        int score = calculadora.calcular(filme, usuario.getPerfil(), catalogoBase);
    ...
    private Recomendacao sortearRecomendacao(List<Filme> filmesFiltrados, Usuario usuario, List<Filme> catalogoBase) {
        int indexSorteado = gerador.sortearInteiro(0, filmesFiltrados.size() - 1);
        Filme filmeSorteado = filmesFiltrados.get(indexSorteado);
        
        int score = calculadora.calcular(filmeSorteado, usuario.getPerfil(), catalogoBase);
